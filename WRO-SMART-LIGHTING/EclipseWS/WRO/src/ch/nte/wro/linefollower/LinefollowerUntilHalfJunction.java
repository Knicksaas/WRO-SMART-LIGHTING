...
public class LinefollowerUntilHalfJunction {
	...
	public LinefollowerUntilHalfJunction(int speed, RegulatedMotor mLeft, RegulatedMotor mRight, int kp) {
		this.kp = kp;
		this.speed = speed;
		this.mLeft = mLeft;
		this.mRight = mRight;
	...
	private void followLine() {
		...
		while(running) {
			...
			if((lLeft.getIntensity() < 0.07)&&(lRight.getIntensity() < 0.42)) {
				new motorsOFF(mLeft, mRight);
				running = false;
			}
			
			if((lRight.getIntensity() < 0.07)&&(lLeft.getIntensity() < 0.42)) {
				new motorsOFF(mLeft, mRight);
				running = false;
			}
		}
